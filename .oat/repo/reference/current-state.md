...
- Review loop:
  ...
  - `oat review latest` resolves the newest project or ad-hoc review artifact by `oat_generated_at` frontmatter, target priority, and lifecycle recency for same-day ties (`final` > higher phase/task > lower phase/task).
...
- Dispatch ceilings are an OAT-owned, **provider-neutral intent** (`dispatch-ceiling-ux`, ADR-019). Repo/user/local config sets `workflow.dispatchCeiling.preset` (`balanced`/`maximum`/`cost-conscious`) and/or explicit `workflow.dispatchCeiling.providers.codex` (`low`/`medium`/`high`/`xhigh`) and `.providers.claude` (`haiku`/`sonnet`/`opus`); a preset compiles **at write time** to concrete per-provider values. Project `state.md` persists the compiled `oat_dispatch_ceiling` (`preset?` + `providers` + `source`); runtime reads only the concrete `providers.*`, never the preset label. The earlier flat `workflow.dispatchCeiling.codex`/`.claude` keys were removed (clean break, no migration).
...
- A **provider adapter registry** decides enforcement per provider, and `oat project dispatch-ceiling resolve` joins stored intent × adapter into per-provider `{value, mode, mechanism, dispatchArgs}` with `mode` (enforced/advisory/unsupported) computed at dispatch and never persisted (plus a verify-on-upgrade flag for above-orchestrator requests). Codex selected effort maps to generated `oat-phase-implementer-{low,medium,high,xhigh}`/`oat-reviewer-{low,medium,high,xhigh}` pinned variants (implementer `min(preferred, ceiling)`, reviewer at the ceiling); Claude enforces via the per-call Task `model` parameter (bidirectional — verified — no variant files); other providers are advisory until an adapter declares a mechanism. Base Codex roles remain provider-default/unpinned fallbacks. Escalation termini are provider-specific: Codex `low→medium→high→xhigh`, Claude Code `haiku→sonnet→opus`.
...
- Archive state and retrieval:
  ...
  - `oat project archive [project-path]` archives a target project through the CLI-owned completion archive helper; omitting the path falls back to the active project.
  - `oat repo archive sync [project-name]` can pull all archived projects, or one named project, down from the configured repo-scoped S3 archive, selecting the latest dated remote snapshot per project. `oat project archive sync` remains as a deprecated shim for the old command path.
