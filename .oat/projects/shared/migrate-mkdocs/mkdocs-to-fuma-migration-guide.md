...
The `@/.source/server` path is generated by `fumadocs-mdx` postinstall. Make sure `postinstall: "fumadocs-mdx"` is in package.json.
...
@layer base {
  ...
  .markdown-alert-note { --callout-color: var(--color-fd-info, #3b82f6); }
  .markdown-alert-tip { --callout-color: #22c55e; }
  .markdown-alert-important { --callout-color: #a855f7; }
  .markdown-alert-warning { --callout-color: #eab308; }
  .markdown-alert-caution { --callout-color: #ef4444; }
}
```
