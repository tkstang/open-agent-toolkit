...
describe('MkDocs scaffold compatibility (FR8)', () => {
  ...
  it('scaffolds MkDocs app with correct structure and config', async () => {
  ...
  it('scaffolds MkDocs app without Fumadocs dependencies', async () => {
    assetsRoot = await bundleAssets();
    ...
    expect(packageJson).not.toContain('@oat/docs-config');
    expect(packageJson).not.toContain('@oat/docs-theme');
    expect(packageJson).not.toContain('@oat/docs-transforms');
  });
});
